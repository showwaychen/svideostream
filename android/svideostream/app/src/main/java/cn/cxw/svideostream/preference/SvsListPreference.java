package cn.cxw.svideostream.preference;
...
public class SvsListPreference  extends ListPreference {
    private CharSequence[] mEntrySummaries;

    public SvsListPreference(Context context) {
        super(context);
        initPreference(context, null);
    }

    public SvsListPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPreference(context, attrs);
    }

    public SvsListPreference(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initPreference(context, attrs);
    }

    public SvsListPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initPreference(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.SvsListPreference, 0, 0);
        if (a == null)
            return;

        mEntrySummaries = a
                .getTextArray(R.styleable.SvsListPreference_entrySummaries);
