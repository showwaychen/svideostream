...
GraphicBuffer::GraphicBuffer(uint32_t width, uint32_t height, PixelFormat format, uint32_t usage):
    library("libui.so")
{
	LOGD << "new instance GraphicBuffer";
    ...
    if (memory == nullptr) {
        LOGE << "Could not alloc for GraphicBuffer";
        return;
    }
...
uint32_t GraphicBuffer::getWidth() const
{
	return ((android::android_native_buffer_t*)getNativeBuffer())->width;

}

uint32_t GraphicBuffer::getHeight() const
{
